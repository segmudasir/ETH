...
// Place this at the top of your script or just before startTracking()
window.addEventListener('DOMContentLoaded', () => {
  fetch('https://staff-api-plz8.onrender.com/data')
    .then(res => res.json())
    .then(data => {
    ...
    if (data.Size !== undefined) {
      ...
      if (data.Leverage !== undefined) {
          leverage = data.Leverage;  // set leverage from API
        } else {
          console.error('Leverage value missing in API response!');
        }
    })
    .catch(err => {
      console.error('Error loading EntryPrice from API:', err);
    });
});
...
async function getETHPrice() {
  ...
  try {
   ...
   if (isTrackingStarted && !isNaN(stopValue) && !stopCrossed) 
   {
    ...
    console.error('Leverage not loaded yet, cannot calculate liquidation price.');
    ...
    // ✅ Update leverage
    document.querySelector('.value[data-type="leverage"]').textContent = `x${leverage}`;
  } catch (error) {
    console.error('Error fetching ETH price:', error);
  }
}
...
>>>>>>> efc9c07 (Update: Entry price fetch from API, leverage UI added)
